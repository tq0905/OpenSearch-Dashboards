...
describe('useApplyQueryAction', () => {
  ...
  it('counts the fetched rows when a PPL response carries no hit total', async () => {
    // The data-frame conversion PPL goes through hardcodes hits.total to 0, so
    // reporting it verbatim would tell the model the query matched nothing.
    const h = setup({ currentLanguage: 'PPL' });
    const action = lastEnabled(h.registered);
