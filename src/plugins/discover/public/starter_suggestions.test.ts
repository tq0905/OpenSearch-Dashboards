...
describe('registerDiscoverStarterSuggestions', () => {
  ...
  beforeEach(() => {
    ...
    handle = registerDiscoverStarterSuggestions(chat);
  });
  ...
  describe('cards per query outcome', () => {
    ...
    it('still names an unknown reason when the error text is missing', () => {
      const cards = cardsFor(withQuery({ query: 'status:200', status: ResultStatus.ERROR }));

      expect(cards[0].prompt).toContain('unknown error');
    });
    ...
    it('puts the result count in the summarize card', () => {
      const cards = cardsFor(
        withQuery({ query: 'status:200', status: ResultStatus.READY, resultsCount: 1204 })
      );

      expect(cards).toHaveLength(1);
      expect(cards[0].id).toBe('summarizeResults');
      expect(cards[0].text).toBe('Summarize these 1,204 results');
    });

    it('omits the count when there is no total', () => {
      expect(cardsFor(withQuery({ query: 'status:200', status: ResultStatus.READY }))[0].text).toBe(
    ...
    it('falls back to a generic prompt when the query text is empty', () => {
      ...
      expect(cards[0].prompt).toBe('My query failed. Help me understand the error and fix it.');
    });
  });
  ...
  describe('page context lookup', () => {
    ...
    it("picks Discover's entry out of a crowded context list", () => {
      ...
      expect(cardsFor(context)[0].id).toBe('summarizeResults');
    });
